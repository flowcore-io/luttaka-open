...
export const UpdateConferenceForm: FC<UpdateConferenceProps> = ({conference, close, refetch}) => {
  ...
  const onSubmit = useCallback(async (values: UpdateConferenceInput) => {
    ...
    const valuesToSubmit: UpdateConferenceInput = {
      ...
      description: conference.description !== values.description ? values.description : undefined,
      ticketCurrency: conference.ticketCurrency !== values.ticketCurrency ? values.ticketCurrency : undefined,
      ticketPrice: conference.ticketPrice !== values.ticketPrice ? values.ticketPrice : undefined,
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit, (errors, event) => {
        ...
        toast.error(`Failed to update conference with ${JSON.stringify(errors)}`);
      })} className={"space-y-3"}>
        <FormField
          ...
          <FormItem>
            ...
            <FormControl>
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  {codes.map((code) => (
                    <SelectItem value={code.value} key={code.value}>{code.label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </FormControl>
        ...
        <FormField
          ...
          render={({field}) => (
            <FormItem>
              ...
              <FormControl>
                <NumericFormat placeholder={"price"} value={field.value} onValueChange={(value) => field.onChange({
                  target: {
                    value: value.floatValue,
                    name: field.name,
                  }
                })} suffix={` ${currencyCode}`} customInput={Input}/>
              </FormControl>
              <FormMessage/>
            </FormItem>
          )}
              ...
              <FormControl>
                <div>
                  <DateTimePicker onChange={(value) => {
                    ...
                    if (value.date > new Date(form.watch("endDate") ?? conference.endDate)) {
