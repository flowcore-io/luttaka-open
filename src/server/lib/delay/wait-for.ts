 ...
 */
export async function waitFor<T>(method: () => Promise<T>, expected: (result: T) => boolean): Promise<T | undefined> {

  let finalResult: T | undefined = undefined;

  await retry({times: NUMBER_OF_ATTEMPTS, delay: DELAY_BETWEEN_ATTEMPTS}, async () => {
